...
class Teacher
{
    ...
    public static function schedule($id) {
        ...
        $smbt = \App\PdoHelper::get()->query("SELECT
            ...
            `type_id`,
            `subgroup`,
            `group_id`, (SELECT `name` FROM `groups` WHERE `id` = i.`group_id`) `group`,
            `location`
            FROM event_templates i WHERE `teacher_id` = $id
        ...
        for ($i = 0; $i < count($db_res); $i++) {
            $info = [
                ...
                "group_id" => intval($db_res[$i]->group_id),
                "group" => $db_res[$i]->group,
                "location" => $db_res[$i]->location,

                //ZALIPUHA
                "teacher_id" => intval($db_res[$i]->group_id),
                "teacher" => $db_res[$i]->group,
            ];
